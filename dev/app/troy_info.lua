...
person_handler.troy_index_tbl = "info_index"
person_handler.troy_info_tbl = "info"
person_handler.troy_db = "troy"
...
function person_handler.get_self_info(_word)
	...
	local ret = mysql_client:read_condition_not_check(person_handler.troy_db, person_handler.troy_info_tbl, query_dict)
...
function person_handler.insert_index(_key_word, _word)
	...
	local ret = mysql_client:read_condition_not_check(person_handler.troy_db, person_handler.troy_index_tbl, query_dict)
	ret = ret or {}
	if not ret[1] then
		local insert_dict = {word = _word, key_word = _key_word, weight = 0}
		ret = mysql_client:insert_not_check(person_handler.troy_db, person_handler.troy_index_tbl, insert_dict)
	end
end
...
function person_handler.insert_info(_key_word, _info, _type)
	...
	if not ret[1] then
		...
		ret = mysql_client:insert_not_check(person_handler.troy_db, person_handler.troy_info_tbl, insert_dict, {info="MediumBlob"})
	end
end
...
function person_handler.search_info(_peer_ctx, _word)
	...
	if ret and ret[1] then
		...
		ret = mysql_client:insert_not_check(person_handler.troy_db, person_handler.troy_index_tbl, insert_data, nil, query_dict)
	end
	...
	if info.type == "text" then
		--返回语音合成url
		person_handler.get_baidu_token()
		ret_info.voice_info_list = string.format(person_handler.baidu_voice_composition_url, info.info, person_handler.baidu_voice_token)
	else
		--直接返回语音数据
