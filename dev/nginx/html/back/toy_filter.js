...
function toy_filter_update(id)
{
	...
	sd["id"] = id
	sd["word"] = word.value
	send_data("update_toy_filter", sd)
...
function toy_filter_search()
{
	...
	var sd = new Array
	sd["word"] = word.value
	send_data("search_toy_filter", sd)
...
function toy_filter_from_update()
{
	...
	for(var idx in toy_filter_list)
	{
		...
		ar[i++] = ["b,É¾³ý,toy_filter_del(" + info.id + ")","b,¸üÐÂ,toy_filter_update(" + info.id + ")"]
		tb_ar.push(ar)
	}
